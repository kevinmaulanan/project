            ...
            return new Promise((resolve, reject) => {
                ...
                db.query(query, (error, result, field) => {
                    if (error) {
                        ...
                        const { total } = result[0]
                        const query = `SELECT category_detail.id, category.category, category_detail.category_detail FROM category_detail JOIN category ON category_detail.id_category=category.id ${conditions} ${paginate}`
                        db.query(query, (error, result, field) => {
                            if (error) {
